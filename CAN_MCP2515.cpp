...
void CAN_MCP2515::setBitrate(uint32_t bitrate, uint32_t canClock)
{
	...
	if (!set)
	{
		Serial.print("canClock finally unknown...\r\n");
	}

	writeAddress(MCP2515_CNF1, cfg1); //Write config address 1
	writeAddress(MCP2515_CNF2, cfg2); //Write config address 2
	writeAddress(MCP2515_CNF3, cfg3); //Write config address 3
}
...
#ifdef NORDIC_CAN
CAN_MCP2515 CAN(SPI_SS_PIN); // Create CAN channel using pin NRFX_SPIM_SS_PIN from SPI_nordic.h for SPI chip select
#else
